...
__attribute__((__used__)) int main(int i, char **c)
{
	...
	const unsigned char macadr[6] = {0x10, 0xe2, 0xd5, 0x00, 0x00, 0x00};
	#define TFTP_SERVERPORT 69
		/* Set the extracted MAC address as macadr */
	//set_mac_addr(macadr);
	udp_start(macadr, IPTOINT(local_ip[0], local_ip[1], local_ip[2], local_ip[3]));
	char * filename = NULL;	
        filename = "image.mem";
        char *buffero=(void *)0x40100000;
	int size;
	printf("Copying %s to %p... ", filename, buffero);
	size = tftp_get(ip, TFTP_SERVERPORT, filename, buffero);
	if(size > 0)
		printf("(%d bytes)", size);
	printf("\n");
